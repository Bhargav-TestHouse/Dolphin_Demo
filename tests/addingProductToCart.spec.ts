...
test('Adding product to cart', async () => {
    ...
    await page.locator("(//div[@class='product-action']//button[@title='Add to Cart']//i[@class='fas fa-shopping-cart'])[1]").dblclick();

    await page.locator("//a[text()='View Cart ']").click();
    await expect(page).toHaveScreenshot("CartPage-MaskQuantity.png",  {mask: [page.locator("//div[@class='input-group flex-nowrap']//input")], maxDiffPixelRatio: 0.03});
